...
| `transcription.vote.enabled`                      | true    | global, server  | evaluation    | For low-confidence slow snippets, run prompt and no-prompt paths and keep the better non-trivial candidate.                |
| `transcription.finalPass.enabled`                 | true    | global          | evaluation    | Verify finalized meeting audio and auto-apply high-confidence fixes, while rejecting trivial replacements like `.`.        |
