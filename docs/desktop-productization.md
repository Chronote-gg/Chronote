...
The access boundary is the desktop token and its scopes. Each registered source in a recording is bounded to one `MAXIMUM_MEETING_DURATION` of declared audio. Anything narrower, per-user quotas for instance, belongs in the personal upload path shared with the portal, not in a desktop-only gate.
