...
- Desktop recorder hosted API routes are always registered. Access is gated by `DESKTOP_ALLOWED_USER_IDS`, which falls back to `SUPER_ADMIN_USER_IDS`, and an empty allowlist denies every account outside mock mode. That allowlist is the beta switch; there is deliberately no second on/off flag, because a route group that 404s and an allowlist that 403s were two gates for one decision, and the redundant one is what left the hosted desktop API dark. Set it in production as the `TFVAR_DESKTOP_ALLOWED_USER_IDS` GitHub Actions variable.
