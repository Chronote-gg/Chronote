...
- The consent post is CSRF-blocklisted in `webserver.ts` like the MCP one, because it carries a signed request plus a single use session nonce instead of a CSRF token. The nonce is consumed on first use, so an approval cannot be replayed. The signed token, the nonce and the desktop client's `LOGIN_TIMEOUT_SECONDS` all sit at 5 minutes, so a slow approval fails as one thing rather than half expiring.
