...
- Production keeps legacy VPC flow log resource names (`vpc_flow_logs_role` and `/vpc/flow/app-vpc`) to avoid replacing live logging resources; non-production environments use scoped names. Do not change production `grafana_suffix_seed` without an approved AMG workspace replacement plan.
