...
- **`guildCreate` does not only mean "joined".** It also fires when an existing guild becomes available after an outage, so `server_installed` is gated on `joinedTimestamp` being recent. Without that, an outage recovery reads as a burst of installs and the count drifts upward permanently, with no self-correction. `guildDelete` needs no such guard: the gateway flags outages with `unavailable` and discord.js routes those to `guildUnavailable`.
