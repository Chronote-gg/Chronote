...
- **A desktop recording job is bounded by declared audio duration, not by segment count or total bytes.** `PERSONAL_RECORDING_MAX_TOTAL_MILLIS` is one `MAXIMUM_MEETING_DURATION` per registered source, enforced in `createPersonalRecordingSegmentUploadIntent` before a signed upload is issued. The per-segment byte cap does not bound a job on its own, because nothing caps how many segments a session may add, and that was genuinely unbounded until the beta allowlist came off. Duration is the right unit: a byte cap would have to assume a WAV format the capture device actually chooses, and transcription cost scales with minutes rather than segments.
