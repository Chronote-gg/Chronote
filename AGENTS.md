...
- The `joinedTimestamp` guard on `server_installed` therefore covers a narrower case than an outage: a `GUILD_CREATE` re-emitted for a guild already joined, if it were ever dropped from cache while the client is Ready. It is cheap insurance, not protection from the availability path. Keep it, but do not repeat the earlier claim that outages flow through `guildCreate` (they do not).
