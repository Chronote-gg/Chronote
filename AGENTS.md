...
- `expectedOutput` schemas in the runners must be `.nullish()`, not `.optional()`. Langfuse returns `null` for a case that declares none, and `.optional()` rejects null, which crashes the evaluator on every unlabelled case instead of skipping its exact-match grades.
