...
Recordings with no usable speech skip notes generation. Incomplete transcripts
still produce notes with a separate warning. Processing outcomes are independent
of meeting completion; see [meeting lifecycle](apps/docs-site/docs/core-concepts/meeting-lifecycle.md).
...
- Server owners can independently disable viewer access to transcripts and audio recordings while leaving summaries available.
...
- Hosted desktop recorder routes are open to any signed-in account, gated by desktop OAuth token scopes rather than an allowlist. Desktop recordings use segmented upload state in DynamoDB plus S3 audio objects.
